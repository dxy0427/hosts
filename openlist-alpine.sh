    ...
    case "$choice_reset_pass" in
        ...
        2)
            ...
            if [ $? -eq 0 ]; then
                ...
                echo -e "请使用 'admin' 和新设置的密码登录。"

                local openlist_output_trimmed=$(echo "$openlist_cmd_raw_output" | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
                if [ -n "$openlist_output_trimmed" ] && \
                   ! echo "$openlist_output_trimmed" | grep -Eqi "^admin user has been updated|^success|^succeeded|^已更新|^updated"; then
